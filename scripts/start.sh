...
# bash -c 'source ./install/setup.bash && nohup ros2 launch vision launch.py > vision.log 2>&1 &'
...
bash -c 'source ./install/setup.bash && nohup ros2 launch game_controller launch.py > game_controller.log 2>&1 &'
echo "[START SOUND]"
bash -c 'source ./install/setup.bash && nohup ros2 run sound_play sound_play_node > sound.log 2>&1 &'
echo "[DONE]"
