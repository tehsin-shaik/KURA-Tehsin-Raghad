...
{
    // Match the expected QoS of the low-level Loco API subscriber (avoid DURABILITY mismatch)
    auto qos = rclcpp::QoS(rclcpp::KeepLast(10));
    qos.reliable();
    qos.transient_local();
    publisher = brain->create_publisher<booster_msgs::msg::RpcReqMsg>("LocoApiTopicReq", qos);
}
...
int RobotClient::kickLeg()
{
    int apiId = -1;
    std::string body = "";
    if (!brain->get_parameter("robot.kick_api_id", apiId)) {
        prtWarn("robot.kick_api_id not set; mechanical kick skipped");
        return -1;
    }
    brain->get_parameter("robot.kick_body", body);

    booster_interface::msg::BoosterApiReqMsg msg;
    msg.api_id = static_cast<int64_t>(apiId);
    msg.body = body;
    return call(msg);
}
