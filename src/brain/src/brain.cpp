...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<int>("robot.kick_api_id", -1);
    declare_parameter<std::string>("robot.kick_body", "");
...
void Brain::handleCooperation() {
    ...
    for (int i = 0; i < aliveTmIdxs.size(); i++) {
        ...
        if (!status.ballDetected) continue;

        double dist = norm(status.ballPosToField.x - data->robotPoseToField.x,
                        status.ballPosToField.y - data->robotPoseToField.y);

        // Only consider if the ball is far enough away
        if (dist > RANGE_THRESHOLD) {
            // Prefer the one with highest confidence; if equal, choose nearer one
            if (status.ballConfidence > bestConfidence ||
            (fabs(status.ballConfidence - bestConfidence) < 1e-3 && status.ballRange < minRange)) {

                bestConfidence = status.ballConfidence;
